...
const Money: NextPage = () => {
  ...
  const { user } = useUser();
  const [uploadinquiry, { loading, data }] = useMutation<i>(
    "/api/users/me/virtual"
  );
  ...
  return (
    <Layout hasTabBar canGoBack title="가상 머니 충전">
      <form onSubmit={handleSubmit(onValid)}>
        ...
        <button className="mt-52 rounded-md border-2 border-transparent bg-black px-60 py-3 text-center font-medium text-white hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-gray-700 focus:ring-offset-1 ">
          충전하기
        </button>
        <button className="text-blacks mt-4 rounded-md border-2 border-gray-300 bg-white px-60 py-3 text-center font-medium hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-gray-700 focus:ring-offset-1 ">
          취소하기
        </button>
      </form>
    </Layout>
